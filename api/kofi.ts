...
const CENSOR = '*****'
const KOFI_IMG = 'https://i.imgur.com/J0egcX2.png'
...
async function sendDiscordEmbed(webhookUrl: string, payload: KoFiPayload, kofiUsername?: string) {
  const embed = {
    author: {
      name: 'Ko-fi',
      icon_url: KOFI_IMG,
    ...
    footer: {
      ...
      icon_url: 'https://github.githubassets.com/images/modules/site/icons/funding_platforms/ko_fi.svg',
    },
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!kofiToken) {
    return res.status(400).json({ success: false, error: 'Ko-fi token required.' })
  }
  ...
  if (payload.verification_token !== kofiToken) {
    return res.status(403).json({ success: false, error: 'Ko-fi token does not match.' })
  }
